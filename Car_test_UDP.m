    ...
    while ishandle(stop_fig) 
        ...
        if u_recv.NumDatagramsAvailable > 0
            ...
            if length(raw_bytes) == 32
                ...
                raw_car_x = parsed_doubles(1);
                raw_car_y = parsed_doubles(2);
                
                % 4. Lock the Car Origin on the very first received packet
                if isnan(car_origin_x)
                    car_origin_x = raw_car_x;
                    car_origin_y = raw_car_y;
                    disp('✅ Car Origin Locked! Translating path to (15, 15).');
                end
                
                % 5. Subtract the origin, THEN add the 15.0 offset
                carx      = (raw_car_x - car_origin_x) + car_X_offset;
                cary      = (raw_car_y - car_origin_y) + car_Y_offset;
